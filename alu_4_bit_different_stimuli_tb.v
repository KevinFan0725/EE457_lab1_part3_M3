...
/* Caution: 
We all know that 4'b0110 means six. Some students leave out 4'b and fill in 0110 for the A or the B test values.
Then that 0110 is interpreted as "one hundred and ten" in decimal which is equivalent 
to 1101110 in binary (64+32+8+4+2 = 110). 
After truncating it to the lower four bits, the value applied to A or B is 1110 (4'1110 = fourteen).
Several students in the past made this mistake and wasted several hours in debugging :(
Please use 4'b notation for 4-bit binary numbers here. 
*/
...
task TEST_ALU;
 ...
 reg [58*8:1] comment_string_unsigned, comment_string_signed;
   begin
	   ...
	   case (Opr_value)
			1	: if ((~AINV_value) && BNEG_value) Opr_string_tb = "SUB operation";
...
// test #1 begin
	TEST_ALU (4'b1111 , 4'b0001 , 0, 0, 1); // (A,B,AINV,BNEG,Opr)  ADD
// test #1 end

// test #2 begin
	TEST_ALU (4'b0101 , 4'b0010 , 0, 0, 1); // (A,B,AINV,BNEG,Opr)  ADD
// test #2 end

// test #3 begin
	TEST_ALU (4'b0110, 4'b0101 , 0, 0, 1); // (A,B,AINV,BNEG,Opr)  ADD
// test #3 end

// test #4 begin
	TEST_ALU (4'b1001 ,4'b1001 , 0, 0, 1); // (A,B,AINV,BNEG,Opr)  ADD
// test #4 end

// test #5 begin
	TEST_ALU (4'b0101 ,4'b0011  , 0, 1, 1); // (A,B,AINV,BNEG,Opr)  SUB
// test #5 end

// test #6 begin
	TEST_ALU ( 4'b1001, 4'b0111 , 0, 1, 1); // (A,B,AINV,BNEG,Opr)  SUB
// test #6 end

// test #7 begin
	TEST_ALU (4'b0011 , 4'b0101 , 0, 1, 1); // (A,B,AINV,BNEG,Opr)  SUB
// test #7 end

// test #8 begin
	TEST_ALU (4'b0111 ,4'b1111  , 0, 1, 1); // (A,B,AINV,BNEG,Opr)  SUB
// test #8 end
